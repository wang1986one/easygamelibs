...
//                 compare value lengths instead of using _tcsncmp(,,std::max(..)) which
...
//
// TODO:
//   P4 Change calls to _tcsncmp( ..., std::max(l1,l2) ) to 'l1 == l2 && _tcsncmp( .., l1 )
//      It is more efficient to compare lengths first, than using std::max() and  _tcsncmp().
...
inline static bool load_file(const TCHAR* path,TCHAR** buffer,unsigned long* size,unsigned long tempsize = 4096)
{
	...
	*buffer = 0;
	...
	if(pFile)
	{
		...
		*size=ftell(pFile);
		(*size)++;
		*buffer=(TCHAR*) malloc(sizeof(TCHAR)*(*size));
		fseek(pFile,0,SEEK_SET);
		*size=fread_s(*buffer,sizeof(TCHAR)*(*size),sizeof(TCHAR),*size,pFile);
		(*buffer)[*size]=0;
		fclose(pFile);
	}
...
public:
    ...
    // TODO: update all of these to compare lengths instead of using std::max. Ditto for all uses of std::max() NF 29 Mar 2003 See: xml_attribute_struct::matches_attribute_name_value()
...
#else
						std::min(valuelen,_tcslen(node->value));
#endif
					...
					if(chartype_symbol(*s)) //<... #...
					{
						...
						if(*s!=0 && (chartype_equals(ch) || chartype_equals(*s))) //'<... #=...'
						{
							...
							if(chartype_quote(*s)) //'<... #="...'
							{
                                ...
                                if ( _tcsncmp( a->name, _T("expanded"), std::max( (8*sizeof(TCHAR)), a->name_size ) ) == 0
                                  && _tcsncmp( a->value, _T("true"), std::max( 4*sizeof(TCHAR), a->value_size ) ) == 0
                                   )
                                {
                                    cursor->expand( true );
