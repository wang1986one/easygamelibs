...
DWORD GetFileIndex_Het(TMPQArchive * ha, const char * szFileName)
{
    ...
    while(pHetTable->pHetHashes[Index] != HET_ENTRY_FREE)
    {
        ...
        if(pHetTable->pHetHashes[Index] == HetHash)
        {
            ...
            // where some files were depeted by StormLib. Perhaps 
    ...
    TFileEntry * pFileEntry)
{
    ...
    bool bIsLastFileEntry = true;
    int nError = ERROR_SUCCESS;

    // Determine if this is a last file entry
    for(pTempEntry = pFileEntry + 1; pTempEntry < pFileTableEnd; pTempEntry++)
    {
        // Is the file entry NOT deleted?
        if((pTempEntry->dwFlags & (MPQ_FILE_EXISTS | MPQ_FILE_DELETE_MARKER)) == MPQ_FILE_EXISTS)
        {
            bIsLastFileEntry = false;
            break;
        }
    }

    //
    // If we are deleting the last file entry, we can just get rid of it.
    // However, if we are deleting a middle entry, we need to mark the file as deleted,
    // otherwise the BET table will report the file as existing
    //
    // TODO: Shouldn't we keep the file table as it is (if there's HET table)?
    //

    if(bIsLastFileEntry || ha->pHetTable == NULL)
    {
        TFileEntry * pLastFileEntry = ha->pFileTable + ha->dwFileTableSize - 1;
        TFileEntry * pLastUsedEntry = NULL;
        TFileEntry * pLastFreeEntry = NULL;
        ...
        for(pTempEntry = ha->pFileTable; pTempEntry < pFileTableEnd; pTempEntry++)
        {
            ...
            if((pTempEntry->dwFlags & (MPQ_FILE_EXISTS | MPQ_FILE_DELETE_MARKER)) == MPQ_FILE_EXISTS)
                ...
                pLastFreeEntry = pTempEntry;
        }

        // Can we free some entries at the end?                 
        if(pLastUsedEntry < pLastFileEntry && pLastFreeEntry == pLastFileEntry)
        {
            // Free all file entries that follow
            for(pTempEntry = pLastUsedEntry + 1; pTempEntry < pFileTableEnd; pTempEntry++)
                ClearFileEntry(ha, pTempEntry);
    ...
    {
        // Clear *SOME* of the items in the file table
        // We have to keep BET hash, hash index, HET index, locale and platform
        pFileEntry->ByteOffset = 0;
        pFileEntry->FileTime   = 0;
        pFileEntry->dwFileSize = 0;
        pFileEntry->dwCmpSize  = 0;
